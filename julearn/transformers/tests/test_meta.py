...
from julearn.transformers import JuTransformerWrapper
...
def test_transform_all_return_same_passthrough():
    ...
    trans_df = JuTransformerWrapper(transformer=PassThroughTransformer(),
                                    apply_to='all',
...
def test_pca_transform_all():
    trans_df = JuTransformerWrapper(transformer=PCA(n_components=2,
                                                    random_state=1),
                                    apply_to='all',
                                    returned_features='unknown',
                                    )
...
def test_pca_transform_AB():
    ...
    trans_df = JuTransformerWrapper(transformer=PCA(n_components=1,
                                                    random_state=1),
                                    apply_to=['A', 'B'],
                                    returned_features='unknown',
                                    )
...
def test_scale_columns_of_type_return_same():
    ...
    for condition, columns in condition_columns:
        trans_df = JuTransformerWrapper(transformer=StandardScaler(),
                                        apply_to=condition,
                                        returned_features='same',
                                        )
